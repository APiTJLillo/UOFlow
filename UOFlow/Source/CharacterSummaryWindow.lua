...
CharacterSummaryWindow.ScrollWindowName = "CharacterSummaryWindowScroll"
...
CharacterSummaryWindow.ContentWidth = 410
...
CharacterSummaryWindow.LastDisplayedText = L""
...
function CharacterSummaryWindow.Initialize()
	...
	if DoesWindowNameExist(CharacterSummaryWindow.DisplayLabelName) then
		WindowSetAlpha(CharacterSummaryWindow.DisplayLabelName, 0)
		WindowSetShowing(CharacterSummaryWindow.DisplayLabelName, false)
	end

	if DoesWindowNameExist(CharacterSummaryWindow.CopyBoxName) then
		local contentWidth = CharacterSummaryWindow.ContentWidth or 410
		WindowSetDimensions(CharacterSummaryWindow.CopyBoxName, contentWidth, 40)
		WindowSetAlpha(CharacterSummaryWindow.CopyBoxName, 1)
		WindowSetShowing(CharacterSummaryWindow.CopyBoxName, true)
	end
end
...
function CharacterSummaryWindow.Refresh()
	...
	local lineCount = newlines + 1
	dispatchDebugMessage(string.format("[CharacterSummary] lineCount=%d", lineCount))
	local estimatedHeight = lineCount * 18
	local textHeight = estimatedHeight

	if ensureDisplayLabel() then
		LabelSetText(CharacterSummaryWindow.DisplayLabelName, woutput)
		local _, measuredHeight = LabelGetTextDimensions(CharacterSummaryWindow.DisplayLabelName)
		if measuredHeight and measuredHeight > 0 then
			textHeight = math.max(measuredHeight, estimatedHeight)
		end
		textHeight = math.max(40, textHeight + 10)
		WindowSetDimensions(CharacterSummaryWindow.DisplayLabelName, contentWidth, textHeight)
		local _, currentLabelHeight = WindowGetDimensions(CharacterSummaryWindow.DisplayLabelName)
		dispatchDebugMessage(string.format("[CharacterSummary] Label height: %.0f (est %.0f, lines %d)", currentLabelHeight or -1, textHeight, lineCount))
		WindowSetAlpha(CharacterSummaryWindow.DisplayLabelName, 0)
		WindowSetShowing(CharacterSummaryWindow.DisplayLabelName, false)
	else
		textHeight = math.max(40, estimatedHeight + 10)
	end

	if ensureScrollChild() then
		WindowSetDimensions(CharacterSummaryWindow.ScrollChildName, contentWidth, textHeight + 12)
		local _, currentChildHeight = WindowGetDimensions(CharacterSummaryWindow.ScrollChildName)
		dispatchDebugMessage(string.format("[CharacterSummary] Scroll child height: %.0f", currentChildHeight or -1))
	end

	if CharacterSummaryWindow.ScrollWindowName and DoesWindowNameExist(CharacterSummaryWindow.ScrollWindowName) then
		ScrollWindowUpdateScrollRect(CharacterSummaryWindow.ScrollWindowName)
		ScrollWindowSetOffset(CharacterSummaryWindow.ScrollWindowName, 0)
	end

	if ensureEditBox() then
		TextEditBoxSetText(CharacterSummaryWindow.EditBoxName, woutput)
		WindowSetDimensions(CharacterSummaryWindow.EditBoxName, contentWidth, textHeight)
		WindowSetAlpha(CharacterSummaryWindow.EditBoxName, 1)
	end

	CharacterSummaryWindow.LastDisplayedText = woutput
