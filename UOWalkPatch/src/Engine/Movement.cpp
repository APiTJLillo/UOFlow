...
static constexpr size_t kFastWalkNominalDepth = 32;
...
static constexpr uint32_t kFastWalkJitterBudgetMs = 150;
static constexpr uint32_t kFastWalkResyncInflightCycles = 4;
static constexpr uint64_t kFastWalkHistoryTtlMs = 4000;
static constexpr size_t kFastWalkHistoryLimit = 64;
...
struct AckTracker {
    std::deque<AckPendingEntry> pending;
    bool recovering = false;
};

enum class AckProcessAction {
    Ignore,
    Ok,
    SoftFail,
    Resync
...
struct AckProcessResult {
    AckProcessAction action = AckProcessAction::Ignore;
    uint8_t expected = 0;
    uint32_t dropped = 0;
    uint64_t sentTick = 0;
};
...
static void ResyncFastWalkLocked(SOCKET socket,
    ...
    if (maxInflightOverride > 0)
        Walk::Controller::ApplyInflightOverride(maxInflightOverride, kFastWalkResyncInflightCycles);

    if (reason) {
        Walk::Controller::NotifyResync(reason);
        Log::Logf(Log::Level::Info,
                  Log::Category::FastWalk,
                  "[FW] resync reason=%s socket=%p drop=%zu head=%u tail=%u maxInflight_tmp=%u",
                  reason,
...
static void ResyncFastWalkForProbeMiss(uint32_t key) {
    ...
    ResyncFastWalkLocked(target, g_lastIndexHead, g_lastIndexTail, GetTickCount64(), "probe-miss", 1);
...
static void CheckFastWalkTimeoutsLocked(SOCKET socket, uint32_t headIndex, uint32_t tailIndex, uint64_t now) {
    ...
    auto& history = it->second;
    const auto settings = Walk::Controller::GetSettings();
    uint32_t baselineDelay = settings.stepDelayMs ? settings.stepDelayMs : 250u;
    uint64_t thresholdMs = static_cast<uint64_t>(baselineDelay) * 2ull + kFastWalkJitterBudgetMs;
    ...
    if (triggered) {
        ...
        Log::Logf(Log::Level::Warn,
                  Log::Category::FastWalk,
                  "[FW] delay socket=%p head=%u tail=%u latestAgeMs=%llu thresholdMs=%llu recentKeys=[%s]",
                  reinterpret_cast<void*>(static_cast<uintptr_t>(socket)),
                  headIndex,
                  tailIndex,
                  static_cast<unsigned long long>(maxAge),
                  static_cast<unsigned long long>(thresholdMs),
                  (offset > 0) ? keyBuf : "<none>");
        g_fwMisses.fetch_add(1, std::memory_order_relaxed);
        ResyncFastWalkLocked(socket, headIndex, tailIndex, now, "delay", 1);
    }
...
static void AckTrackerRecordSend(SOCKET socket, uint8_t seq, uint64_t tick) {
    if (seq == 0)
        seq = 1;
    if (socket == INVALID_SOCKET)
        return;
    ...
    AckTracker& tracker = g_ackTrackers[canonical];
    tracker.pending.push_back({seq, tick});
    if (tracker.pending.size() > 64)
        tracker.pending.pop_front();
}

static AckProcessResult AckTrackerProcessAck(SOCKET socket, uint8_t seq, uint8_t status, uint64_t now) {
    ...
    if (!tracker.pending.empty())
        result.expected = tracker.pending.front().seq;
    else
        result.expected = seq;
    ...
    if (status != 0) {
        ...
        result.action = AckProcessAction::SoftFail;
        return result;
    }

    if (tracker.pending.empty()) {
        result.action = AckProcessAction::Ok;
        tracker.recovering = false;
        return result;
    }

    uint32_t dropped = 0;
    AckPendingEntry matched{};
    bool found = false;
    while (!tracker.pending.empty()) {
        AckPendingEntry entry = tracker.pending.front();
        tracker.pending.pop_front();
        if (entry.seq == seq) {
            matched = entry;
            found = true;
    ...
    if (!found) {
        tracker.pending.clear();
        tracker.recovering = false;
        result.action = AckProcessAction::Resync;
        result.dropped = dropped;
        return result;
    }

    result.sentTick = matched.sentTick;
    result.dropped = dropped;
    tracker.recovering = false;
    if (!tracker.pending.empty())
        result.expected = tracker.pending.front().seq;
    else
        result.expected = static_cast<uint8_t>(seq + 1);
...
static void TrackMovementTxInternal(uint8_t seq, int dir, bool run, SOCKET socket, uint32_t key, const char* sender)
{
    ...
    AckTrackerRecordSend(socket, seq, entry.tick);
...
void ResyncFastWalk(SOCKET socket, const char* reason, uint32_t maxInflightOverride) {
    ...
    {
        std::lock_guard<std::mutex> lock(g_fastWalkMutex);
        canonical = ResolveSocketAliasLocked(socket);
        if (canonical == INVALID_SOCKET)
            canonical = socket;
        headIndex = g_lastIndexHead;
        tailIndex = g_lastIndexTail;
        ResyncFastWalkLocked(canonical, headIndex, tailIndex, now, reason, maxInflightOverride);
    }

    {
        std::lock_guard<std::mutex> ackLock(g_ackMutex);
        if (canonical == INVALID_SOCKET) {
            g_ackTrackers.clear();
        } else {
            g_ackTrackers.erase(canonical);
        }
    }
}
...
void OnSocketClosed(SOCKET socket) {
    if (socket == INVALID_SOCKET)
        return;
    SOCKET canonicalSocket = INVALID_SOCKET;
    {
        std::lock_guard<std::mutex> lock(g_fastWalkMutex);
        canonicalSocket = ResolveSocketAliasLocked(socket);
        ...
        if (canonicalSocket != INVALID_SOCKET && canonicalSocket != socket) {
            g_fastWalkQueues.erase(canonicalSocket);
            g_fastWalkExpectedDepth.erase(canonicalSocket);
            g_fastWalkKeyHistory.erase(canonicalSocket);
        }
    ...
    {
        std::lock_guard<std::mutex> ackLock(g_ackMutex);
        g_ackTrackers.erase(socket);
        if (canonicalSocket != INVALID_SOCKET && canonicalSocket != socket)
            g_ackTrackers.erase(canonicalSocket);
    }
}
...
MovementAckResult ProcessMovementAck(SOCKET socket, uint8_t seq, uint8_t status)
{
    ...
    AckProcessResult internal = AckTrackerProcessAck(socket, seq, status, nowTick);
    MovementAckResult result{};
    result.expected = internal.expected ? internal.expected : static_cast<uint8_t>(seq + 1);
    result.dropped = internal.dropped;
    switch (internal.action) {
    case AckProcessAction::SoftFail:
        result.action = MovementAckAction::Drop;
        break;
    case AckProcessAction::Resync:
        result.action = MovementAckAction::Resync;
        break;
    case AckProcessAction::Ok:
        result.action = MovementAckAction::Ok;
        break;
    default:
